...
const getPokemonsAPI = async (max, min) => {
  try {
    const poke = await axios.get(
      `https://pokeapi.co/api/v2/pokemon?limit=${max}&offset=${min}`
    );
    ...
    const pokemons = await Promise.all(promises).then((values) => {
      return values.map((pk) => ({
        ...
        image:
          pk.data.sprites.versions["generation-v"]["black-white"].animated
            .front_default,
        imageBack:
          pk.data.sprites.versions["generation-v"]["black-white"].animated
            .back_default,
      ...
      return values.map((pk) => ({
        ...
        imageBack:
          pk.data.sprites.versions["generation-v"]["black-white"].animated
            .back_default,
        imageFrontDefault:
          pk.data.sprites.other["official-artwork"].front_default,
        types: pk.data.types.map((t) => {
          return {
            id: t.type.url.split("/")[6],
            name: t.type.name,
